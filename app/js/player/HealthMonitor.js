...
var HealthMonitor = (function () {
  ...
  function HealthMonitor(videoEl, opts) {
    ...
    /* Folga depois de uma busca. Avancar reabre a conexao e o video fica
       parado ate o servidor responder pelo novo ponto; isso NAO e
       travamento. O nodecast trata 'waiting' como estado normal (mostra
       spinner e deixa o motor se recuperar) e nao tem watchdog nenhum;
       o Hypnotix entrega o problema ao mpv. Como aqui existe watchdog,
       ele precisa saber a diferenca — senao avancar um filme vira
       "canal fora do ar" depois de duas tentativas. */
    this.seekGraceMs = opts.seekGraceMs || 12000;
    this.timer = null;
    this.reset();
  ...
  /* Chamado pelo orquestrador quando o proprio app pede uma busca, e pelo
     evento "seeking" do elemento — cobre retomada, controles e qualquer
     outro caminho que mexa em currentTime. */
  HealthMonitor.prototype.noteSeek = function () {
    var t = Date.now();
    this.seekUntil = t + this.seekGraceMs;
    this.lastMove = t;
  };

  /* usado quando o backend manda startupMs para este item */
  HealthMonitor.prototype.setStartup = function (ms) {
    this.startupFixo = ms || null;
  };
  ...
  HealthMonitor.prototype._check = function () {
    ...
    if (this.firstFrameMs === null) {
      ...
      /* prazo fixo tem prioridade: o backend sabe se e arquivo grande
         sem faststart, que passa muito de 8s com readyState em 0 */
      var limite;
      if (this.startupFixo) { limite = this.startupFixo; }
      else {
        limite = this.startupByReady[this.v.readyState];
        if (limite === undefined) { limite = 12000; }
      }
      if (!this.startupFired && el > limite && t >= this.seekUntil) {
      ...
      if (this.v.seeking) { this.seekUntil = Math.max(this.seekUntil, t + this.seekGraceMs); }
      if (ct > this.lastCT + 0.001) {
        this.lastCT = ct;
        this.lastMove = t;
        if (this.stalled) { this.stalled = false; this.onRecover(); }
      } else if (!this.v.paused && !this.v.ended && t >= this.seekUntil) {
        if (!this.stalled && (t - this.lastMove) > this.stallAfter) {
          this.stalled = true;
          this.stallCount++;
          this.onStall({ frozenFor: t - this.lastMove, ct: ct, count: this.stallCount,
                         readyState: this.v.readyState, networkState: this.v.networkState,
                         seeking: !!this.v.seeking });
        }
      }
    }
    ...
    if (t - this.lastReport >= this.report) {
      ...
      this.onTick({
        ...
        seeking: !!this.v.seeking,
        emFolgaDeBusca: t < this.seekUntil,
