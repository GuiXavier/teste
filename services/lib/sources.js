...
function XtreamProvider(cfg) {
  ...
  this.authData = null;
  this.container = cfg.container === "ts" ? "ts" : "m3u8";
}
...
XtreamProvider.prototype._api = function (action, params, cb) {
  ...
  opts.headers = config.headers(null, this.headers);
  ...
  function fin(e, d) {
    ...
    if (tTotal) { clearTimeout(tTotal); }
    cb(e, d);
  }
  /* req.setTimeout do Node e por INATIVIDADE. Uma resposta de 100 MB
     chegando devagar nunca dispara, e a chamada fica pendurada para
     sempre — foi o que aconteceu com get_vod_streams no teste de 23:02.
     Este e o teto de tempo TOTAL. */
  tTotal = setTimeout(function () {
    try { req.destroy(); } catch (e) {}
    fin(new Error("excedeu o tempo total (" + Math.round((opts.tempoMax || 45000) / 1000) + "s)"));
  }, opts.tempoMax || 45000);
  ...
  var req = mod.request(opts, function (r) {
    ...
    r.on("data", function (c) {
      b += c;
      if (b.length > 4e7) {   /* 40 MB: alem disso nao e lista, e problema */
        try { req.destroy(); } catch (e) {}
        fin(new Error("resposta grande demais (" + Math.round(b.length / 1e6) + " MB)"));
      }
    });
    r.on("end", function () {
...
XtreamProvider.prototype.getCategories = function (cb) {
  this._api("get_live_categories", {}, function (err, d) {
    if (err) { return cb(err); }
    if (!Array.isArray(d) || !d.every(function (row) { return row && typeof row === "object" && (!row.container_extension || typeof row.container_extension === "string"); })) { return cb(new Error("resposta de categorias invalida")); }
...
XtreamProvider.prototype.getChannels = function (opts, cb) {
  ...
  this._api("get_live_streams",
    ...
    function (err, d) {
      if (err) { return cb(err); }
      if (!Array.isArray(d) || !d.every(function (row) { return row && typeof row === "object" && (!row.container_extension || typeof row.container_extension === "string"); })) { return cb(new Error("resposta de catalogo invalida")); }
  ...
  this._api("get_vod_streams",
    ...
    function (err, d) {
      ...
      cb(null, d.map(function (v) {
        ...
        return {
          id: "v" + v.stream_id, streamId: v.stream_id, kind: "vod", headers: self.headers,
...
XtreamProvider.prototype.getSeriesInfo = function (seriesId, cb) {
  ...
  this._api("get_series_info", { series_id: seriesId }, function (err, d) {
    if (err) { return cb(err); }
    if (!d || !d.episodes || typeof d.episodes !== "object" || Array.isArray(d.episodes) && d.episodes.length) { return cb(new Error("serie sem resposta completa")); }

    var temporadas = [];
    var eps = d.episodes || {};
    if (!Object.keys(eps).every(function (key) { return Array.isArray(eps[key]) && eps[key].every(function (ep) { return ep && ep.id !== undefined && ep.id !== null && (!ep.container_extension || typeof ep.container_extension === "string"); }); })) {
      return cb(new Error("lista de episodios invalida"));
    }
    var k;
    for (k in eps) {
      ...
      var lista = (eps[k] || []).map(function (e) {
        ...
        var info = e.info || {};
        return {
          id: "e" + e.id, episodeId: e.id, kind: "episode", headers: self.headers,
