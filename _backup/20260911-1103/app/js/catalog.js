...
var Catalog = (function () {
  ...
  /* origem usada pela sincronizacao: JS Service se responder, senao PC */
  function origem(cb) {
    if (lunaDisponivel === true) { return cb(Sync.origemLuna); }
    if (lunaDisponivel === false) { return cb(Sync.origemHttp); }
    Luna.service("ping", {}, function (r, e) {
      lunaDisponivel = !e && !!r;
      cb(lunaDisponivel ? Sync.origemLuna : Sync.origemHttp);
    }, 3000);
  }
  ...
  function play(id, cb) {
    ...
    achar(id, function (it) {
      ...
      base.fallback = ehTs ? null : (API.getBase() ? "hlsjs" : null);
  ...
  function sincronizar(fonte, onProg, cb) {
    origem(function (org) {
      /* a lista de fontes do servidor vem SEM senha (sanitizada). Se o
         JS Service estiver disponivel mas a senha nao, o Xtream falharia
         na TV; nesse caso o PC, que tem a senha, sincroniza. */
      if (org === Sync.origemLuna && fonte.type === "xtream" && !fonte.password && API.getBase()) {
        Log.info("catalog", "sem senha local: sincronizando pelo PC");
        org = Sync.origemHttp;
      }
