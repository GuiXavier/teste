...
(function () {
  ...
  function loadCategoryNow(i) {
    ...
    API.channels({ category: cat.id, limit: PAGINA, offset: 0 }, function (err, data) {
      if (err) {
      ...
      list.setSource(data.total, function (off, lim, cb) {
        ...
        API.channels({ category: cat.id, limit: lim, offset: off }, function (e2, d2) {
          if (e2) {
  ...
  function onKey(ev) {
    ...
    if (k === Keys.GREEN) { loadCategory(catIndex); return; }
  }
  ...
  function boot() {
    ...
    list = new ChannelList($("channels"), {
      ...
      onActivate: function (ch) { playChannel(ch, false); },
      onPage: function (p) {
