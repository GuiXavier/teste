...
(function () {
  ...
  function scheduleRetry(reason) {
    ...
    Log.warn("retry", "tentativa " + retryCount + "/" + MAX_RETRY + " em " + wait + "ms",
             { channel: currentChannel ? currentChannel.name : null, reason: reason });
  ...
  function resolveBackend(cb) {
    ...
    API.health(function (e, h) {
      if (e) { Log.error("api", "health falhou", { why: e.message }); return cb(false); }
  ...
  function trocarSecao(delta) {
    ...
    API.categories({ type: sec.id }, function (err, cats) {
  ...
  function abrirSerie(item) {
    ...
    API.seriesInfo(item.id, function (err, info) {
      if (err) { setStatus("is-err", "erro: " + err.message); return; }
      var eps = [];
  ...
  function loadCategoryNow(i) {
    ...
    API.channels({ type: tipo, category: cat.id, limit: PAGINA, offset: 0 }, function (err, data) {
      if (err) {
      ...
      list.setSource(data.total, function (off, lim, cb) {
        ...
        API.channels({ type: tipo, category: cat.id, limit: lim, offset: off }, function (e2, d2) {
          if (e2) {
  ...
  function playChannelNow(ch, isReload) {
    ...
    var t = now();
    API.play(ch.id, function (err, info) {
  ...
  function onKey(ev) {
    ...
    if (k === Keys.BLUE) {
      ...
      Settings.open(function () {
        ...
        API.categories(function (err, cats) {
          if (err) { return; }
          categories = cats; catIndex = 0; renderCats(); loadCategoryNow(0);
        });
      });
    ...
    resolveBackend(function (ok) {
      if (!ok) {
        ...
        overlay("<div>Backend nao encontrado.<br><span style='font-size:var(--fs-sm)'>" +
                "Rode <b>node server.js</b> e confira o IP em app.js.</span></div>");
        return;
      }
      API.categories(function (err, cats) {
