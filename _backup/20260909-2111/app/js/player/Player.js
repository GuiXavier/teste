...
var Player = (function () {
  ...
  Player.prototype._bind = function () {
    var self = this;
    function add(n, fn) { self.v.addEventListener(n, fn, false); self.listeners.push({ n: n, fn: fn }); }
  ...
  Player.prototype._fail = function (why) {
    var self = this;
    if ((this.engine === "native") && !this.triedFallback &&
        this.current && this.current.fallback === "hlsjs") {
      this.triedFallback = true;
      this._emit("statechange", { state: "fallback", why: why });
      setTimeout(function () { self._startHls(); }, 120);
      return;
    }
  ...
  Player.prototype.switchToFallback = function (why) {
    if (this.triedFallback || !this.current || this.current.fallback !== "hlsjs") { return false; }
    this.triedFallback = true;
    this._emit("statechange", { state: "fallback", why: why });
    this._startHls();
    return true;
  ...
  Player.prototype._startNative = function () {
    ...
    this.v.src = this.current.direct;
    this.v.load();
    var p = this.v.play();
    ...
    this.hls.on(Hls.Events.ERROR, function (e, d) {
      ...
      if (d.type === Hls.ErrorTypes.NETWORK_ERROR) {
        ...
        if (self.netRetry <= 3) {
          ...
          setTimeout(function () { if (self.hls) { try { self.hls.startLoad(); } catch (x) {} } }, wait);
          return;
        }
