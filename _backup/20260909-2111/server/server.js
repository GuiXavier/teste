...
function reload(cb) {
  ...
  STATE.movies = []; STATE.movieCategories = [];
  STATE.series = []; STATE.seriesCategories = [];
  STATE.provider = null;
...
function carregarVod(prov, cb) {
  if (typeof prov.getVod !== "function") { return cb(); }
  prov.getVodCategories(function (e1, cats) {
    prov.getVod({}, function (e2, filmes) {
      if (e2 || !filmes) {
        console.log("[xtream] filmes indisponiveis" + (e2 ? (": " + e2.message) : ""));
        return cb();
      }
      STATE.movies = filmes;
      STATE.movieCategories = nomearCategorias(filmes, cats);
      var naoTocaveis = filmes.filter(function (f) { return !f.playable; }).length;
      console.log("[xtream] " + filmes.length + " filmes, " +
                  STATE.movieCategories.length + " categorias" +
                  (naoTocaveis ? ("  (" + naoTocaveis + " em container que a TV nao toca)") : ""));
      cb();
    });
  });
}
...
function carregarSeries(prov, cb) {
  if (typeof prov.getSeries !== "function") { return cb(); }
  prov.getSeriesCategories(function (e1, cats) {
    prov.getSeries({}, function (e2, sers) {
      if (e2 || !sers) {
        console.log("[xtream] series indisponiveis" + (e2 ? (": " + e2.message) : ""));
        return cb();
      }
      STATE.series = sers;
      STATE.seriesCategories = nomearCategorias(sers, cats);
      console.log("[xtream] " + sers.length + " series, " +
                  STATE.seriesCategories.length + " categorias" +
                  "  (temporadas carregadas sob demanda)");
      cb();
    });
  });
}
...
function colecao(tipo) {
  if (tipo === "vod") { return { itens: STATE.movies, cats: STATE.movieCategories }; }
  if (tipo === "series") { return { itens: STATE.series, cats: STATE.seriesCategories }; }
  return { itens: STATE.channels, cats: STATE.categories };
}
...
function rotear(req, res, u) {
  ...
  if (p === "/api/health") {
    return json(res, 200, {
      ...
      movies: STATE.movies.length, movieCategories: STATE.movieCategories.length,
      series: STATE.series.length, seriesCategories: STATE.seriesCategories.length,
  ...
  if (p === "/api/channels") {
    ...
    var filtered = colL.itens.filter(function (c) {
  ...
  if (p.indexOf("/api/play/") === 0) {
    ...
    var ch = null, i;
    var todos = STATE.channels.concat(STATE.movies).concat(STATE.episodeCache || []);
