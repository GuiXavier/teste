...
var Catalog = (function () {
  ...
  function origem(cb) {
    ...
    if (lunaDisponivel === true) { return decide(); }
    /* A primeira chamada ao servico inclui o tempo de a webOS SUBIR o
       Node: no log de 11/09 levou 3145ms e estourou o limite de 3s,
       enquanto a segunda respondeu em 24ms. Prazo generoso, e um "nao"
       NUNCA fica em cache — so o "sim". */
    Luna.service("ping", {}, function (r, e) {
      if (!e && r) {
        lunaDisponivel = true;
        Log.info("catalog", "JS Service disponivel (Node " + r.node + ", " + r.rssMB + " MB)");
      } else {
        lunaDisponivel = null;   /* tenta de novo na proxima */
        Log.warn("catalog", "JS Service nao respondeu agora", { why: e });
      }
      decide();
    }, 12000);
  }
  ...
  function play(id, cb) {
    ...
    achar(id, function (it) {
      if (!it) {
        /* nao esta no banco (episodio de serie ainda nao aberta, item de
           categoria nao sincronizada). Se o PC estiver ai, ele resolve. */
        if (API.getBase() && !API.simularOffline()) {
          Log.info("catalog", "item fora do banco: resolvendo pelo PC", { id: id });
          return API.play(id, cb);
        }
        return cb(new Error("item nao esta no banco local; abra a categoria/serie primeiro"));
      }
