...
var Sync = (function () {
  ...
  var origemLuna = {
    ...
    itens: function (src, tipo, categoria, offset, limit, cb) {
      ...
      Luna.service("xtream", p, function (r, e) {
        if (e) { return cb(e); }
        /* o JS Service devolve o Xtream CRU (stream_id, container_extension);
           o servidor HTTP ja normaliza. Aqui normalizamos para o formato do
           banco: id com prefixo, url montada, playable. */
        var itens = (r.items || []).map(function (x) {
          return normalizarXtream(x, tipo, src);
        });
        cb(null, { total: r.total, items: itens });
      });
    },
    serieInfo: function (src, id, cb) {
      ...
      Luna.service("xtream", p, function (r, e) {
        if (e || !r || !r.dados) { return cb(e || "serie sem dados"); }
        var d = r.dados, temporadas = [], eps = d.episodes || {}, k;
        for (k in eps) {
          if (!eps.hasOwnProperty(k)) { continue; }
          var lista = (eps[k] || []).map(function (ep) {
            var ext = (ep.container_extension || "mp4").toLowerCase();
            var info = ep.info || {};
            return { id: "e" + ep.id, episodeId: ep.id, kind: "episode",
                     name: ep.title || ("Episodio " + ep.episode_num),
                     num: parseInt(ep.episode_num, 10) || 0, season: parseInt(k, 10) || 0,
                     plot: info.plot || null, container: ext,
                     playable: (ext === "mp4" || ext === "m3u8" || ext === "ts"),
                     url: urlXtream(src, ep.id, "series", ext), status: "unknown" };
          });
        ...
        cb(null, { info: d.info || {}, seasons: temporadas });
      });
    }
  };

  /* Normaliza um item cru do Xtream (vindo do JS Service) para o formato
     que o banco espera. Espelha o que o server.js faz no lado do PC. */
  function urlXtream(src, streamId, tipo, container) {
    var base = String(src.url || "").replace(/\/+$/, "");
    if (base.indexOf("http") !== 0) { base = "http://" + base; }
    var seg = { live: "live", vod: "movie", series: "series" }[tipo] || "live";
    return base + "/" + seg + "/" + src.username + "/" + src.password +
           "/" + streamId + "." + (container || "m3u8");
  }
  function normalizarXtream(x, tipo, src) {
    if (tipo === "live") {
      return { id: "x" + x.stream_id, streamId: x.stream_id, kind: "live",
               name: x.name, logo: x.stream_icon || null,
               category: String(x.category_id), tvgId: x.epg_channel_id || null,
               catchup: String(x.tv_archive) === "1" ? "xtream" : null,
               url: urlXtream(src, x.stream_id, "live", "m3u8"), status: "unknown" };
    }
    if (tipo === "vod") {
      var ext = (x.container_extension || "mp4").toLowerCase();
      return { id: "v" + x.stream_id, streamId: x.stream_id, kind: "vod",
               name: x.name, logo: x.stream_icon || x.cover || null,
               category: String(x.category_id), rating: x.rating || null,
               container: ext, playable: (ext === "mp4" || ext === "m3u8" || ext === "ts"),
               url: urlXtream(src, x.stream_id, "vod", ext), status: "unknown" };
    }
    /* series: so a ficha; episodios vem no serieInfo */
    return { id: "s" + x.series_id, seriesId: x.series_id, kind: "series",
             name: x.name, logo: x.cover || null, category: String(x.category_id),
             plot: x.plot || null, rating: x.rating || null, status: "unknown" };
  }
      ...
      gravarCategorias(src, "live", cats, function () {
        ...
        baixarTudo(src, origem, "live", "__all__", "live", function (a, b) { prog("canais ao vivo", a, b); },
          function (e2, r) {
            ...
            /* SALVAGUARDA: so poda se a lista nova veio inteira e sem erro.
               No log de 11/09 uma sincronizacao com a simulacao ligada
               devolveu 0 canais e a poda apagou os 3332 do banco. Um
               catalogo bom nunca deve ser destruido por uma falha de rede. */
            function seguirPosPoda(n) {
              relato.podados = n || 0;
            ...
            /* Poda so quando a lista nova e confiavel: sem erro E com
               itens. Lista vazia ou com falha significa problema de
               rede, nao provedor removendo canais. */
            if (e2 || !r.gravados) {
              relato.erros.push("poda cancelada: lista vazia ou com erro (banco preservado)");
              DB.count("live", src.id, null, function (e9, n) {
                relato.live = n || 0;   /* relata o que REALMENTE tem no banco */
                seguirPosPoda(0);
              });
            } else {
              DB.pruneSource("live", src.id, r.ids, seguirPosPoda);
            }
          });
      });
    });
