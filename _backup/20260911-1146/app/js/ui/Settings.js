...
var Settings = (function () {
  ...
  function render() {
    ...
    estado.sources.forEach(function (s) {
      var row = h('<div class="set-src' + (s.active ? " is-active" : "") + '">' +
        ...
        '<div class="set-src-meta">' + s.type +
          ...
          (s.hasPassword ? " &middot; senha no PC" : "") + "</div></div>");
      src.appendChild(row);
      campos.push({
    ...
    botao(dia, (API.simularOffline() ? "SIMULANDO servidor desligado - desligar simulacao"
                                     : "Simular servidor desligado (telemetria continua)"), function () {
      var on = !API.simularOffline();
      API.simularOffline(on);
      Log.info("ui", "simulacao servidor desligado: " + (on ? "LIGADA" : "desligada"));
      Log.stat("simul", on ? "OFFLINE" : "");
      msg(on ? "simulando PC desligado. Feche a configuracao e use o app; o log continua chegando."
             : "simulacao desligada", "ok");
      render();
    });
    ...
    botao(dia, "Sincronizar catalogo para a TV", function () {
      ...
      var fonte = { id: ativa.id, type: ativa.type, name: ativa.name, url: ativa.url,
                    ...
                    /* a senha NAO vem do servidor (sanitizada). Se voce a
                       digitar no campo "Senha" antes de sincronizar, ela fica
                       guardada na TV e o JS Service passa a buscar sozinho. */
                    password: (inpPass && inpPass.value) || null };
      if (!fonte.password) {
        var jaTem = Catalog.getFonte();
        if (jaTem && jaTem.id === fonte.id && jaTem.password) { fonte.password = jaTem.password; }
      }
      var t0 = Date.now();
      msg("sincronizando " + fonte.name + " ...", "wait");
      Log.info("sync", "iniciando", { fonte: fonte.id });

      /* Sem senha na TV? Pede ao PC antes de sincronizar. Assim o JS
         Service passa a autenticar sozinho depois, sem digitar nada. */
      function seguir() { Catalog.sincronizar(fonte, progresso, fim); }
      if (fonte.type === "xtream" && !fonte.password && !API.simularOffline()) {
        API.sourceSecret(fonte.id, function (e, sec) {
          if (!e && sec && sec.password) {
            fonte.password = sec.password;
            if (sec.username) { fonte.username = sec.username; }
            Log.info("sync", "senha obtida do PC e guardada na TV");
          } else {
            Log.warn("sync", "PC nao entregou a senha", { why: e ? e.message : "vazia" });
          }
          seguir();
        });
      } else { seguir(); }
    ...
    linha(dia, "Senha na TV", fl ? (fl.password ? "sim" : "NAO - sincronize com o PC ligado") : "-");
    linha(dia, "Simulacao offline", API.simularOffline() ? "LIGADA" : "-");
    if (fl && estado.resumo) {
